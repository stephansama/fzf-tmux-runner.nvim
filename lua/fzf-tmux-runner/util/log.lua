...
function log.notify(scope, level, verbose, str, ...)
    if not verbose and _G.FzfTmuxRunner.config ~= nil and not _G.FzfTmuxRunner.config.debug then
        return
    end
    ...
    vim.notify(
        string.format("[fzf-tmux-runner.nvim@%s] %s", scope, string.format(str, ...)),
        level,
        { title = "fzf-tmux-runner.nvim" }
    )
end
...
function log.warn_deprecation(options)
    ...
    if uses_deprecated_option then
        ...
        log.notify(
            ...
            vim.log.levels.WARN,
            true,
            "use `:h FzfTmuxRunner.options` to read more."
        )
    end
end
